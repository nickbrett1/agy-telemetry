...
# "agy-telemetry-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "agy-telemetry-dev" is overwritten by
...
PROJECT_NAME="agy-telemetry"
AGENT_NAME="agy-telemetry-dev"
WORKSPACE="/workspaces/agy-telemetry"
...
if [ -x "/workspaces/agy-telemetry/scripts/agent-dev.sh" ]; then
  "/workspaces/agy-telemetry/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
